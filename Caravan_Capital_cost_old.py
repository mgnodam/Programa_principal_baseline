...
if __name__ == "__main__":
    ...
    for case in ("loan", "lease", "own"):
        result = compute_capital_cost(params, case)  # type: ignore[arg-type]
        print(f"Scenario: {case}")
        print(f"  Annual capital cost:  US$ {result.annual_cost_usd:,.0f}")
        print(f"  Cost per flight hour: US$ {result.hourly_cost_usd:,.0f}/h")
        print(f"  Cost per NM:         US$ {result.cost_per_nm_usd:.2f}/nm\n")
